...
namespace ifd {
	...
	bool FolderNode(const char* label, ImTextureID icon, bool& clicked)
	{
		...
		if (hovered || active)
			window->DrawList->AddRectFilled(ImGui::GetItemRectMin(), ImGui::GetItemRectMax()/*window->DC.LastItemRect.Min, window->DC.LastItemRect.Max*/, ImGui::ColorConvertFloat4ToU32(ImGui::GetStyle().Colors[active ? ImGuiCol_HeaderActive : ImGuiCol_HeaderHovered]));
	...
	bool FavoriteButton(const char* label, bool isFavorite)
	{
		...
		float size = ImGui::GetItemRectMax().x - ImGui::GetItemRectMin().x/*window->DC.LastItemRect.Max.x - window->DC.LastItemRect.Min.x*/;
	...
	bool FileIcon(const char* label, bool isSelected, ImTextureID icon, ImVec2 size, bool hasPreview, int previewWidth, int previewHeight)
	{
		...
		if (hovered || active || isSelected)
			window->DrawList->AddRectFilled(ImGui::GetItemRectMin(), ImGui::GetItemRectMax()/*window->DC.LastItemRect.Min, window->DC.LastItemRect.Max*/, ImGui::ColorConvertFloat4ToU32(ImGui::GetStyle().Colors[active ? ImGuiCol_HeaderActive : (isSelected ? ImGuiCol_Header : ImGuiCol_HeaderHovered)]));
